...
Le bouton "Telecharger une backup" permet de sauvegarder tous les fichiers dans un dossier "Backup_Spiralydata_YYYY-MM-DD_HH-MM-SS".

**Mode Host:**
```go
// Copie recursive locale
func copyDirRecursive(src, dst string) error
```

**Mode User:**
```go
// Telecharge depuis le serveur via downloadChan
func (c *Client) DownloadBackup(destDir string)
```

### Fonctionnement

- **Host**: Copie les fichiers locaux vers Backup_Spiralydata_date
- **User**: Active `downloadActive`, envoie une requete "request_all_files" au serveur, recoit tous les fichiers via `downloadChan` et les sauvegarde dans Backup_Spiralydata_date

### Format du dossier
```
Backup_Spiralydata_2025-01-31_14-30-45/
```
